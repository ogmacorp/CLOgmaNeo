__kernel void encoder_accum_activation(
    ...
    // Project
    int2 visible_center = (int2)((hidden_column_pos + 0.5f) * hToV);
    ...
    for (int c = 0; c < hidden_size.z; c++) {
        ...
        for (int ix = iter_lower_bound.x; ix <= iter_upper_bound.x; ix++)
            for (int iy = iter_lower_bound.y; iy <= iter_upper_bound.y; iy++) {
                ...
                int visible_state = visible_state[visible_column_index];
...
__kernel void encoder_learn(
    ...
    // Project
    int2 hidden_center = (int2)((visible_column_pos + 0.5f) * vToH);
    ...
    if (max_index != target_state) {
        for (int c = 0; c < visible_size.z; c++) {
            ...
            float delta = lr * ((c == target_state) - exp(reconstruction[visible_cell_index]));
